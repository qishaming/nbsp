...
function initBubbleLog(){
	if(squl){
	    var dom = '<div style="display:none"><div style="width: 276px; height: 190px; overflow: hidden;"><div id="popup_login_box" class="rules_main"><div class="rules_head"><h3>会员登录</h3><a class="exit_btn" href="#"><img alt="" src="https://security.40017.cn/cn/img/comm/images/book/cs_book_common.png"/></a></div><div class="wrap_login_form"><div id="sign_in_form_box_popup"><div class="sign_fm"><form id="login_form_pop" action=""><p id="sign_in_tips_pop" class="sign_tip" style="visibility:hidden;">此账号已注册，请您先登录</p><div class="item"><label for="account_popup">帐号：</label><input id="account_pop" name="name" type="text" maxlength="30" /></div><div class="item"><label for="actpwd_popup">密码：</label><input id="actpwd_pop" name="pass" type="password" maxlength="30" /><a class="sign_lostfound" target="_blank" href="http://Passport.17u.cn/Passport/GetPassword/">忘记密码？</a></div><div class="item"><input type="checkbox" id="rem_it_1w_pop" value="7" name="remIt" /><label class="ri1_label" for="rem_it_1w_pop">记住密码（一周）</label></div><div class="item set_overflow"><button id="sign_in_btn_pop" class="btn_sign_in" title="登录"></button><span id="logining_pop" style="display: none;" class="wait4sign"><img alt="" src="https://security.40017.cn/cn/img/new_ui/public/images/wait.gif" />登录中…</span><a id="cancel_pop" class="cancel_sign" href="###">取消</a></div></form></div></div></div></div></div></div>'
		document.write(dom);
	...
	process: function(project) {
		if(project){
			...
			function modifyImgAndHref(projectImgName) {
				squl.setStyle({background: 'url(https://security.40017.cn/cn/img/new_ui/public/images/' + projectImgName + '5.png) no-repeat 0 0'}, imgsWillChange[4]);
				squl.setStyle({background: 'url(https://security.40017.cn/cn/img/new_ui/public/images/' + projectImgName + '6.png) no-repeat 0 0'}, imgsWillChange[5]);
			}
			switch(project) {
				case '酒店':
